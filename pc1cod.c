...
int main(int argc,char* argv[]) {
    ...
    // "abcdefghijklmnopqrstuvwxyz01234" is the default password used
    // if the user enter a key < 32 characters, characters of the default password will be used 

    strcpy(cle,"abcdefghijklmnopqrstuvwxyz01234");

    printf ("PC1 Cipher 256 bits \nENCRYPT file\n");
    printf("Enter a password upto 32 charecters:");
    scanf("%s",buff);
    ...
    while ( (c=fgetc(in)) != EOF) {
        ...
        fputc(0x25+d,out); //we write the two 4 bits parts as ASCII letters 
        fputc(0x25+e,out);
